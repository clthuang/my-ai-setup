...
- [ ] Orchestration prose ≤ 5,000 words, measured by the census `scope_words_orchestration` metric (orchestration baseline: **60,082** words, 2026-07-25; full-tree `scope_words_total` baseline 111,225 stays reported for continuity). Scope resolution at implementation (2026-07-25): the full commands+skills tree includes ~22k words of domain-knowledge packs that Non-Goals forbid rewriting, so a literal full-tree ≤5,000 is unsatisfiable — the target binds the orchestration subset, whose member list is pinned in `scripts/verbosity-census.sh` (DOMAIN_SKILLS / DOMAIN_COMMANDS exclude lists). Verified by `scripts/verbosity-census.sh` (pins the exact grep pattern per metric and the test-file scope).
