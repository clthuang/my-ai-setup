...
## Task Review Iteration 3 - 2026-03-03T01:25:00+08:00

**Decision:** Needs Revision

**Issues:**
- [blocker] Task 5.1.1: Done-when criterion references usage output and non-zero exit, but scaffolding has no __main__ block (that's Task 5.3.3) -> Changed to import-only check via `python -c "import entity_registry.frontmatter_inject"`
- [blocker] Task 5.3.2: References TestFrontmatterInjectCLI class placeholder but that class is created in Task 7.1.1, not Phase 5 -> Changed to "near bottom of test_frontmatter.py after last existing test class"
- [warning] Task 4.1.1: Merge test uses ambiguous set-notation shorthand; engineer could write full-header test that doesn't test merge -> Added explicit "partial dict" language with rationale about relying on merge
- [warning] Task 5.2.1: Edge case test for empty entity_id missing expected return value -> Added `returns ("", None)`

**Changes Made:**
- Task 5.1.1: Changed done-when to import-only verification with PYTHONPATH
- Task 5.3.2: Changed to standalone comment near bottom of file with note about Task 7.1.1 superseding it
- Task 4.1.1: Clarified merge-preserve test with explicit partial dict and merge reliance rationale
- Task 5.2.1: Added expected return value `("", None)` for empty entity_id edge case

---

## Task Review Iteration 4 - 2026-03-03T01:20:00+08:00

**Decision:** Needs Revision

**Issues:**
- [warning] Task 4.1.1: merge-add test still uses set notation `{…, "feature_id": new_fid}` on line 155 while merge-preserve was already fixed to explicit dict on line 154 -> Use explicit dict `{"entity_uuid": same_uuid, "artifact_type": same_at, "feature_id": new_fid}` consistently
- [warning] Task 5.1.1: Done-when says "with PYTHONPATH set to hooks/lib/" but no exact command showing the full PYTHONPATH prefix -> Show full command: `PYTHONPATH=plugins/iflow/hooks/lib plugins/iflow/.venv/bin/python -c "import entity_registry.frontmatter_inject"`

**Changes Made:**
- Task 4.1.1: Changed merge-add test from set notation to explicit dict `{"entity_uuid": same_uuid, "artifact_type": same_at, "feature_id": new_fid}`
- Task 5.1.1: Added full `PYTHONPATH=plugins/iflow/hooks/lib` prefix to done-when command
- Task 5.1.2: Also added same PYTHONPATH prefix for consistency

---

## Task Review Iteration 5 - 2026-03-03T01:25:00+08:00

**Decision:** Needs Revision (cap reached — iteration 5/5)

**Issues:**
- [warning] Task 5.3.3: Done-when verification commands missing PYTHONPATH prefix — module-level imports (`from entity_registry.frontmatter import ...`) execute before `__main__` guard, causing `ModuleNotFoundError` without PYTHONPATH -> Prepend `PYTHONPATH=plugins/iflow/hooks/lib` to both verification commands

**Changes Made:**
- Task 5.3.3: Added `PYTHONPATH=plugins/iflow/hooks/lib` prefix to both done-when verification commands

---

## Step 2: Chain Review Iteration 1 - 2026-03-03T01:30:00+08:00

**Reviewer:** phase-reviewer (gatekeeper)
**Decision:** Needs Revision

**Issues:**
- [warning] PYTHONPATH prefix inconsistent: Tasks 5.1.1/5.1.2 use explicit prefix, but Task 5.2.2 done-when omits it and relies on pytest path discovery -> Prefix all done-when pytest invocations with `PYTHONPATH=plugins/iflow/hooks/lib`
- [suggestion] Task 4.1.1 merge-preserve test bullet could mislead about write_frontmatter accepting incomplete headers -> Add cross-reference to merge semantics (TD-8)

**Changes Made:**
- Task 5.2.2: Added `PYTHONPATH=plugins/iflow/hooks/lib` prefix to done-when pytest command

---

LAZY-LOAD-WARNING: code-simplifier did not confirm artifact reads

---

## Iteration 1 - 2026-03-03T04:30:00+08:00
...
**Issues:**
- [warning] [readability] code-quality-reviewer: test_frontmatter.py imports private internals (_UUID_V4_RE, _parse_block, _serialize_header) without annotation signaling coupling intent (at: test_frontmatter.py:10-23)
  Suggestion: Group internal-symbol imports separately with a comment
- [warning] [readability] code-quality-reviewer: frontmatter_inject.py main() uses string-match 'UUID mismatch' on ValueError message for dispatch — fragile coupling to human-readable string (at: frontmatter_inject.py:179-184)
  Suggestion: Introduce dedicated FrontmatterUUIDMismatch(ValueError) exception class
- [warning] [kiss] code-quality-reviewer: write_frontmatter opens file three times (binary guard, text read, tempfile write) — adds cognitive overhead (at: frontmatter.py:222-235)
  Suggestion: Read raw bytes once, decode manually for text pass
- [suggestion] [readability] code-quality-reviewer: Stray development note at test_frontmatter.py:916
- [suggestion] [readability] code-quality-reviewer: test_ac15_idempotent missing encoding='utf-8' on open() calls
- [suggestion] [formatting] code-quality-reviewer: Phase section comments skip Phase 6 entirely
- [suggestion] [yagni] code-quality-reviewer: _FIELD_ORDER_SET pre-computed frozenset unnecessary for 9 elements
- [suggestion] [design] implementation-reviewer: re.IGNORECASE instead of .lower() — functionally equivalent
- [suggestion] [design] implementation-reviewer: Single try block vs per-step — cleaner, correct behavior
- [suggestion] [config] security-reviewer: Temp file permissions may differ from original (0600 vs 0644)
- [suggestion] [exposure] security-reviewer: TOCTOU gap acknowledged in design
- [suggestion] [exposure] security-reviewer: UUID values in error messages (non-sensitive)

**Changes Made:**
- Added FrontmatterUUIDMismatch(ValueError) exception class in frontmatter.py
- Replaced string-match dispatch in frontmatter_inject.py with typed exception catch
- Refactored read_frontmatter and write_frontmatter from multi-open to single-open pattern
- Annotated private imports in test_frontmatter.py with coupling-intent comment
- Updated AC-16 test to assert FrontmatterUUIDMismatch specifically

---

## Iteration 2 - 2026-03-03T04:45:00+08:00
...
**Issues:**
- [warning] [readability] code-quality-reviewer: Two bare open().read() calls in frontmatter.py:185,228 not using context managers (at: frontmatter.py:185,228)
  Suggestion: Use `with open(filepath, 'rb') as f: raw = f.read()` for both sites
- [suggestion] [readability] code-quality-reviewer: _full_header() helper placement after first use
- [suggestion] [kiss] code-quality-reviewer: optional_kwargs truthiness vs `is not None` asymmetry
- [suggestion] [formatting] code-quality-reviewer: Duplicated SCRIPT_PATH/PYTHONPATH class attributes

**Changes Made:**
- Fixed both bare open() calls in frontmatter.py to use context managers

---

## Iteration 3 - 2026-03-03T05:00:00+08:00
...
**Issues:**
- [warning] [readability] code-quality-reviewer: Bare open() in test_frontmatter.py:524-527 not using context managers (at: test_frontmatter.py:524-527)
  Suggestion: Use context managers for all open() calls in tests
- [warning] [readability] code-quality-reviewer: db.close() in finally block — db could be unbound if EntityDatabase constructor raises, causing NameError (at: frontmatter_inject.py:194)
  Suggestion: Initialize db = None before try block, guard close with `if db is not None`

**Changes Made:**
- Fixed bare open() in test_frontmatter.py to use context managers
- Added db = None initialization before try block in frontmatter_inject.py
- Added `if db is not None:` guard before db.close() in finally block

---

## Iteration 4 - 2026-03-03T05:15:00+08:00
...
**Issues:**
- [warning] [readability] code-quality-reviewer: re.match() vs re.fullmatch() inconsistency in UUID validation — _parse_block uses fullmatch but validate_header uses match (at: frontmatter.py:119)
  Suggestion: Use re.fullmatch() consistently
- [warning] [readability] code-quality-reviewer: _full_header and _write_file/_write_binary helpers defined mid-file after test classes that don't use them (at: test_frontmatter.py:464)
  Suggestion: Move shared test helpers to top of file after constants
- [warning] [flow] code-quality-reviewer: db.close() flow with sys.exit(0) is misleading — process exits immediately on DB failure, finally block never reached (at: frontmatter_inject.py:139-144)
  Suggestion: Add comment explaining why cleanup is unnecessary on failure path

**Changes Made:**
- Changed re.match() to re.fullmatch() in frontmatter.py for consistency
- Moved _write_file, _write_binary, _full_header helpers to top of test file
- Added flow comment explaining sys.exit(0) path in frontmatter_inject.py

---

## Iteration 6 - 2026-03-03T02:45:00+08:00 [FINAL VALIDATION]

**Implementation Review:** Approved (passed with trivial fix applied)
  - Level 1 (Tasks): pass
  - Level 2 (Spec): pass
  - Level 3 (Design): pass
  - Level 4 (PRD): pass
...
**Issues:**
- [warning] [spec] implementation-reviewer: re.IGNORECASE flag on _UUID_V4_RE inconsistent with database.py .lower() convention (at: frontmatter.py:59)
  Suggestion: Remove re.IGNORECASE, use .lower() before fullmatch — reviewer noted "no change required" but fixed for consistency
- [suggestion] [readability] code-quality-reviewer: Consider extracting repeated build_header call pattern into test fixture (at: test_frontmatter.py)
  Suggestion: Create shared fixture for common valid header construction
- [suggestion] [readability] code-quality-reviewer: Explicit listing of all 6 artifact types in test could use parameterize (at: test_frontmatter.py:TestArtifactTypeValidation)
  Suggestion: Use @pytest.mark.parametrize for artifact type iteration
- [suggestion] [readability] code-quality-reviewer: _full_header helper duplicates build_header defaults (at: test_frontmatter.py)
  Suggestion: Consider using build_header directly in tests
- [suggestion] [readability] code-quality-reviewer: Test file at 1497 lines is large but well-organized (at: test_frontmatter.py)
  Suggestion: Could split into test_frontmatter_read.py and test_frontmatter_write.py in future
- [suggestion] [exposure] security-reviewer: Atomic write preserves original file permissions but does not set restrictive permissions on new temp file (at: frontmatter.py:301-313)
  Suggestion: Consider explicit os.chmod after os.rename if files contain sensitive data
- [suggestion] [exposure] security-reviewer: No explicit UnicodeDecodeError handling for non-UTF-8 files (at: frontmatter.py:194,238)
  Suggestion: Wrap .decode("utf-8") in try/except for graceful handling

**Changes Made:**
- Removed re.IGNORECASE from _UUID_V4_RE regex compilation
- Added .lower() before _UUID_V4_RE.fullmatch() in validate_header to align with database.py convention
- All 96 tests pass after fix
