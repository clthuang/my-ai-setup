...
- [ ] Verify the engine path ran (not fallback): create a temp copy with `2>/dev/null` stripped and run it: `sed 's/" 2>\/dev\/null)/")/' plugins/iflow/hooks/yolo-stop.sh > /tmp/yolo-stop-debug.sh && OUTPUT=$(echo '{"stop_hook_active":false}' | bash /tmp/yolo-stop-debug.sh 2>/tmp/hook-stderr.txt); cat /tmp/hook-stderr.txt`. If stderr is empty, the engine path ran without exceptions. If stderr shows Python tracebacks, the fallback caught an exception — investigate
