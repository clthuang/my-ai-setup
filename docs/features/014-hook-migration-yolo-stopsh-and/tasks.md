...
- [ ] Verify the engine path ran (not fallback): temporarily remove the `2>/dev/null` from the end of the NEXT_PHASE block in `yolo-stop.sh` (change `" 2>/dev/null)` to `")`) and re-run: `OUTPUT=$(echo '{"stop_hook_active":false}' | bash plugins/iflow/hooks/yolo-stop.sh 2>/tmp/hook-stderr.txt); cat /tmp/hook-stderr.txt`. If stderr is empty, the engine path ran without exceptions. If stderr shows Python tracebacks, the fallback caught an exception — investigate. After verifying, restore `2>/dev/null` with `git checkout plugins/iflow/hooks/yolo-stop.sh` and re-apply the combined invocation change

**Done when:** Parsed `reason` field contains `"Invoke /iflow:design"` AND the temporary stderr check confirms no Python exceptions (engine path ran cleanly). If the output shows a different phase, uses the fallback path, or has no reason field, investigate before proceeding.
