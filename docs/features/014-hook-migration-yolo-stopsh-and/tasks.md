...
- [ ] Locate the block starting with `NEXT_PHASE=$(python3 -c "` and ending with `" 2>/dev/null)` (pre-migration lines 172-184) — match by the `phase_map = {` pattern inside
- [ ] Replace the entire block (lines 172-184) with the combined Python invocation from design.md C1:
...
- [ ] Run `bash plugins/iflow/hooks/tests/test-hooks.sh`
- [ ] Compare results against Phase 0 baseline — same tests must pass
...
- [ ] Run the hook from the project root: `bash plugins/iflow/hooks/yolo-stop.sh`. The hook reads `.meta.json` from the active feature directory and outputs JSON to stdout
- [ ] Inspect the JSON output and confirm the `reason` field contains `"Invoke /iflow:design"` — this proves the engine path derived `specify → design` via PHASE_SEQUENCE index lookup (not the fallback phase_map)

**Done when:** Hook JSON output `reason` field contains `"Invoke /iflow:design"`. If the output shows a different phase or uses the fallback path, investigate before proceeding.
