...
### Step 1a.1: derive_kanban() function
...
### Step 1a.2: Replace all kanban-setting code paths
- **Files:** `feature_lifecycle.py` (STATUS_TO_KANBAN), `workflow_state_server.py` (FEATURE_PHASE_TO_KANBAN usages), `engine.py` (hydration kanban backfill)
- **Work:** Replace all direct kanban_column sets with `derive_kanban()` calls. Remove `STATUS_TO_KANBAN` and `FEATURE_PHASE_TO_KANBAN` constants.
- **Test:** Existing kanban tests pass. New integration test: complete_phase → verify kanban_column matches derive_kanban output.
- **ACs:** AC-4
- **Dependencies:** 1a.1

### Step 1a.3: Field validation in init_feature_state
...
### Step 1a.4: Remove frontmatter health check
- **File:** `reconciliation.py`, `workflow_state_server.py`
...
### Step 1a.5: Maintenance mode for meta-json-guard
...
### Step 1a.6: Artifact completeness warning
- **File:** `workflow_state_server.py` (`_process_complete_phase`)
- **Work:** On `phase="finish"`, check expected artifacts per mode (standard: spec.md, tasks.md, retro.md; full: + design.md, plan.md). Return warnings in result, don't block completion.
- **Test:** Complete standard feature missing retro.md → succeeds with warning. Complete with all artifacts → no warning.
- **ACs:** AC-5
- **Dependencies:** None

### Step 1a.7: Reconciliation reporting
...
### Step 1b.1: Schema migration — combined table rebuilds
...
### Step 1b.2: Update VALID_ENTITY_TYPES
- **File:** `database.py`
...
### Step 1b.3: EntityDatabase new methods
- **File:** `database.py`
...
### Step 1b.4: Central ID generator
...
### Step 1b.5: MCP tool ref parameter
...
### Step 1b.6: Dependency cycle detection
...
### Step 1b.7: WEIGHT_TEMPLATES registry
...
### Step 1b.8: Gate parameterisation for light weight
...
### Step 2.1: Secretary mode detection
...
### Step 2.2: Secretary entity registry queries
- **File:** `commands/secretary.md`
...
### Step 2.4: Universal work creation flow
- **File:** `commands/secretary.md`
...
### Step 3.1: promote_task MCP tool
...
### Step 3.2: DependencyManager + Rollup integration
...
### Step 3.3: EntityWorkflowEngine (minimal, feature + task backends)
- **File:** New `plugins/pd/hooks/lib/workflow_engine/entity_engine.py`
- **Work:** Strategy pattern wrapping frozen WorkflowStateEngine. FeatureBackend delegates to frozen engine. TaskBackend handles task mini-lifecycle. `complete_phase()` runs cascade: unblock → rollup → notify (single BEGIN IMMEDIATE transaction).
- **Test:** Feature complete_phase → delegates to frozen engine + triggers cascade. Task complete_phase → 5D transition + cascade. Cascade unblock + rollup in single transaction.
- **ACs:** AC-25
- **Dependencies:** 3.1, 3.2, 2.3

### Step 3.4: Agent-executable task query
...
### Step 4.1: FiveDBackend for EntityWorkflowEngine
...
### Step 4.2: Project progress derivation
- **File:** `rollup.py`
...
### Step 4.3: Cascade unblock at Deliver gate
- **File:** `entity_engine.py`, `dependencies.py`
- **Work:** On entity completion, `cascade_unblock()` removes from all dependents' blocked_by. If dependent has no remaining deps AND status=blocked → status=planned.
- **Test:** B blocked by A and C. A completes → B still blocked. C completes → B unblocks, status=planned.
- **ACs:** AC-29
- **Dependencies:** 3.3, 1b.6

### Step 4.4: Orphan guard on abandonment
...
### Step 5.1: Initiative and Objective entity lifecycle
- **File:** `entity_engine.py`
...
### Step 5.2: Key Result entity with scoring
...
### Step 5.4: OKR anti-pattern detection
- **File:** `commands/secretary.md`
...
### Step 6.2: Catchball — parent intent on creation
...
### Step 6.3: Entity tagging + circle-aware queries
...
### Step 6.4: Cross-level progress view
