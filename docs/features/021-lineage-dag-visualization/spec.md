...
- Clickable nodes using URL link syntax (no `securityLevel: 'loose'`)
...
4. Emit click handlers: `click {safe_id} "/entities/{type_id}"` for every entity EXCEPT the current one
5. Emit classDef blocks for entity types + `current` style
6. Emit class assignments: `current` for the entity itself, entity_type for all others

**AC:**
- AC-R1.1: Returns valid Mermaid flowchart syntax
...
3. Append `_` + first 4 hex chars of SHA-256 hash of original `type_id` to prevent collisions
...
1. **Increase children depth:** Change line 162 from `db.get_lineage(type_id, "down", 1)` to `db.get_lineage(type_id, "down", 10)`
...
3. **Add to context:** Add `"mermaid_dag": mermaid_dag` to the template context dict (line 171-178)
...
**Changes to Lineage card (currently lines 58-84):**
