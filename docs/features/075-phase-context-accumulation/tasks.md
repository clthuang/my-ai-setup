...
### Task 1.1: Add phase_summaries to METADATA_SCHEMAS (C6)
**File:** `plugins/pd/hooks/lib/entity_registry/metadata.py`
**Change:** Add `"phase_summaries": list,` to `METADATA_SCHEMAS['feature']` dict (after `"weight": str,` at line 44).
**Time:** 5 min
**Done:** METADATA_SCHEMAS['feature'] contains `"phase_summaries": list`
**Depends on:** none

### Task 1.2: Write test for validate_metadata accepting phase_summaries (AC-11)
**File:** `plugins/pd/hooks/lib/entity_registry/test_metadata.py` (or appropriate test file)
**Change:** Add test asserting `validate_metadata("feature", {"phase_summaries": [{"phase": "specify", "timestamp": "2026-04-02T08:00:00Z", "outcome": "Done", "artifacts_produced": ["spec.md"], "key_decisions": "Chose X", "reviewer_feedback_summary": "LGTM", "rework_trigger": None}]})` returns no warnings for the `phase_summaries` key.
**Time:** 5 min
**Done:** Test passes, no schema-mismatch warning for phase_summaries
**Depends on:** Task 1.1

### Task 2.1: Add phase_summaries projection to _project_meta_json (C3)
...
```
**Time:** 5 min
**Done:** `_project_meta_json` includes `phase_summaries` in .meta.json output when present in entity metadata
**Depends on:** none

### Task 2.2: Write test for _project_meta_json phase_summaries projection (AC-3, AC-10)
...
1. Read existing `phase_summaries` from .meta.json (in conversation context)
...
**Change:** Add conditional Phase Context injection template to spec-reviewer and phase-reviewer dispatch prompts:
...
### Task 7.1: Write test for _project_meta_json phase_summaries projection (AC-3)
**File:** `plugins/pd/mcp/test_workflow_state_server.py`
**Note:** May already be covered by Task 2.2. If so, mark as complete with reference.
**Time:** 5 min (skip if done in 2.2)
**Done:** Test exists and passes
**Depends on:** Tasks 2.1, 2.2

### Task 7.2: Write test for validate_metadata phase_summaries acceptance (AC-11)
**File:** `plugins/pd/hooks/lib/entity_registry/test_metadata.py`
**Note:** May already be covered by Task 1.2. If so, mark as complete with reference.
**Time:** 5 min (skip if done in 1.2)
**Done:** Test exists and passes
**Depends on:** Tasks 1.1, 1.2

### Task 7.3: Write integration test for update_entity phase_summaries append (AC-1)
...
### Task 7.4: Write test for summary storage failure non-blocking (AC-2)
**File:** `plugins/pd/mcp/test_workflow_state_server.py`
...
### Task 7.5: Write test for zero behavior change without summaries (AC-10)
**File:** `plugins/pd/mcp/test_workflow_state_server.py`
...
1.1 → 1.2
2.1 → 2.2
1.1 + 2.1 → 3.1 → 3.2 → 3.3 → 3.4 → 4.1
3.4 → 5.1 → 5.2 → 5.3
5.3 → 6.1, 6.2, 6.3, 6.4  (parallel)
All implementation → 7.1, 7.2, 7.3, 7.4, 7.5 → 8.1
```
...
|-------|-------|-----------|------|
| Stage 1: Infrastructure | 1.1, 1.2, 2.1, 2.2 | 25 min | Python code + tests |
...
| Stage 4: Testing | 7.1-7.5, 8.1 | 50 min | Tests + regression |
| **Total** | **20 tasks** | **~3 hours** | |
