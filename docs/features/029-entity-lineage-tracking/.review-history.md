...
## Iteration 1 - 2026-02-27T14:30:00Z
...
**Issues:**
- [blocker] [Level 1] implementation-reviewer: README_FOR_DEV.md not updated with entity registry documentation — no mention of entity registry MCP server, its 6 tools, show-lineage command, or entity database; architecture diagram still shows 'Memory · 2 MCP tools' (at: README_FOR_DEV.md)
  Suggestion: Add entity-registry section documenting MCP server, 6 tools, DB location, backfill, and show-lineage command; update architecture diagram to show entity registry
- [warning] [quality] code-quality-reviewer: export_lineage_markdown uses N+1 Python-side recursion instead of single CTE query (at: database.py:385-441)
  Suggestion: Rewrite _export_tree to use a single recursive CTE that returns depth, then format in Python
- [warning] [quality] code-quality-reviewer: _scan_brainstorms has duplicated register+set_parent code in both .prd.md and .md loops (at: backfill.py:98-149)
  Suggestion: Extract a _register_brainstorm helper
- [warning] [quality] code-quality-reviewer: _scan_backlog silently skips malformed rows with no logging (at: backfill.py:64-95)
  Suggestion: Add stderr logging for skipped rows
- [warning] [quality] code-quality-reviewer: _process_export_lineage_markdown makedirs fails on bare filename (at: server_helpers.py:192-230)
  Suggestion: Guard os.makedirs with check for non-empty dirname
- [warning] [security] security-reviewer: _export_tree unbounded Python recursion, no depth guard (at: database.py:419-441)
  Suggestion: Add max_depth parameter with default 50
- [warning] [security] security-reviewer: _render_node unbounded Python recursion (at: server_helpers.py:63-89)
  Suggestion: Add max_depth parameter with default 50
- [warning] [security] security-reviewer: resolve_output_path doesn't validate path containment — arbitrary file writes possible (at: server_helpers.py:114-137,220-230)
  Suggestion: After resolving, verify path is within artifacts_root using os.path.commonpath or startswith on realpath

**Changes Made:**
- README_FOR_DEV.md: Added Entity Registry subgraph to architecture diagram (6 MCP tools, entities.db), added show-lineage to commands table, added "Entity Registry" documentation section
- database.py _export_tree: Rewrote from N+1 Python-side recursion to single recursive CTE with max_depth=50 guard; depth-based indentation computed from SQL result
- database.py _export_tree: Added truncation detection — checks if children exist beyond max_depth and appends "... (depth limit reached)" indicator
- server_helpers.py _render_node: Added depth and max_depth parameters (defaults 0 and 50); truncates with "... (depth limit reached)" when depth >= max_depth
- server_helpers.py render_tree: Updated call to _render_node to pass depth=0 and max_depth
- server_helpers.py resolve_output_path: Added path containment validation using os.path.realpath + startswith check; returns None if resolved path escapes artifacts_root
- server_helpers.py _process_export_lineage_markdown: Guarded os.makedirs with non-empty dirname check; added error message for path containment rejection
- backfill.py: Extracted _register_brainstorm helper to eliminate duplicated register+set_parent code across .prd.md and .md loops
- backfill.py _scan_backlog: Added stderr logging for malformed rows (non-separator rows with <3 cells)
- test_server_helpers.py: Updated resolve_output_path tests to use tmp_path; added path traversal, absolute-inside-root, and absolute-outside-root test cases
- All 177 tests passing after changes

---

## Iteration 2 - 2026-02-27T14:30:00Z
...
**Issues:**
- [blocker] [Level 2] implementation-reviewer: AC-5 depends_on_features annotations not rendered in project descendant tree — backfill.py stores metadata correctly but _format_entity_label in server_helpers.py:56-64 ignores metadata field entirely (at: server_helpers.py:56-64)
  Suggestion: Parse entity metadata JSON, check for depends_on_features key, and append [depends on: feature:030] annotations to the label
- [blocker] [Level 3] implementation-reviewer: Design I7 depends_on_features Handling not implemented in tree rendering — same root cause as AC-5 (at: server_helpers.py:56-64)
  Suggestion: Same fix as AC-5 — _format_entity_label must read metadata and render dependency annotations
- [warning] [quality] code-quality-reviewer: MIGRATIONS type annotation uses bare `callable` instead of `Callable` (at: database.py:72)
  Suggestion: Use `collections.abc.Callable` or `typing.Callable`
- [warning] [quality] code-quality-reviewer: update_entity MCP tool omits artifact_path parameter — asymmetry with DB layer which supports it (at: entity_server.py:203-232)
  Suggestion: Add artifact_path parameter to the MCP tool to match EntityDatabase.update_entity
- [suggestion] [quality] code-quality-reviewer: children map rebuilds set comprehension on every iteration O(n^2) (at: server_helpers.py:45)
  Suggestion: Pre-build the set of type_ids once before the loop
- [suggestion] [quality] code-quality-reviewer: _safe_set_parent silently swallows ValueError with bare pass (at: backfill.py:403-411)
  Suggestion: Add stderr logging for the swallowed error
- [suggestion] [quality] code-quality-reviewer: error message doesn't include rejected path (at: server_helpers.py:259-260)
  Suggestion: Include the rejected path in the error message
- [suggestion] [security] security-reviewer: raw exception messages in error strings — acceptable for local tooling (at: server_helpers.py:218)
- [suggestion] [security] security-reviewer: max_depth has no upper bound validation on MCP tool (at: entity_server.py:181)

**Changes Made:**
- server_helpers.py _format_entity_label: Added AC-5/I7 depends_on_features annotation — parses entity metadata JSON, checks for depends_on_features list, appends [depends on: feature:xxx, feature:yyy] to label when present; gracefully handles missing/malformed metadata
- database.py: Fixed type annotation from bare `callable` to `Callable[[sqlite3.Connection], None]` (from collections.abc)
- entity_server.py update_entity: Added `artifact_path: str | None = None` parameter to MCP tool, passed through to _db.update_entity()
- test_server_helpers.py: Added 7 new tests in TestFormatEntityLabelDependsOn class (single dep, multiple deps, no metadata, metadata without key, empty list, tree output integration, invalid JSON)
- All 184 tests passing after changes (was 177, +7 new)

---

## Iteration 3 - 2026-02-27T15:30:00Z
...
**Issues:**
- [suggestion] [Level 3] implementation-reviewer: Index name deviation idx_parent_type_id vs design spec idx_parent — functionally identical (at: database.py:67)
- [suggestion] [Level 3] implementation-reviewer: update_entity artifact_path parameter beyond design spec — reasonable extension (at: database.py:319, entity_server.py:208)
- [warning] [quality] code-quality-reviewer: O(N^2) set comprehension in render_tree children-map build — rebuilds set on every iteration (at: server_helpers.py:45)
  Suggestion: Replace with `parent in by_id` since by_id is already fully populated
- [warning] [quality] code-quality-reviewer: _safe_set_parent silently swallows all ValueError including legitimate errors (at: backfill.py:406-411)
  Suggestion: Log swallowed errors to stderr for diagnosability
- [suggestion] [quality] code-quality-reviewer: get_lineage uses bare else for direction==down — invalid direction silently falls through (at: database.py:268-271)
- [suggestion] [quality] code-quality-reviewer: Error message for path escapes doesn't include rejected path (at: server_helpers.py:279)
- [suggestion] [quality] code-quality-reviewer: backlog column mapping fragile without comments (at: backfill.py:96-97)
- [suggestion] [quality] code-quality-reviewer: Inline imports in test_backfill.py test methods are repetitive (at: test_database.py)

**Changes Made:**
- server_helpers.py render_tree: Replaced O(N^2) set comprehension `parent in {e["type_id"] for e in entities}` with `parent in by_id` (by_id already fully populated at that point)
- backfill.py _safe_set_parent: Replaced bare `except ValueError: pass` with stderr logging of the swallowed error for diagnosability
- All 184 tests passing after changes

---

## Iteration 4 - 2026-02-27T16:00:00Z
...
**Issues:**
- [suggestion] [quality] code-quality-reviewer: create-project command name parameter could use slug-derived default (at: commands/create-project.md)
- [suggestion] [quality] code-quality-reviewer: backlog column mapping would benefit from inline comment (at: backfill.py:96-97)
- [suggestion] [quality] code-quality-reviewer: double-split optimization possible in _scan_backlog (at: backfill.py:80-82)

**Changes Made:**
- No changes required — all issues are suggestions only, O(N^2) and silent error fixes from iteration 3 resolved all warnings

---

## Iteration 5 - 2026-02-27T16:30:00Z [FINAL VALIDATION]

**Implementation Review:** Approved
  - Level 1 (Tasks): PASS — all 43 tasks verified
  - Level 2 (Spec): PASS — all 14 ACs verified
  - Level 3 (Design): PASS — 1 suggestion (index name idx_parent_type_id vs design spec idx_parent, cosmetic)
  - Level 4 (PRD): PASS — all 4 goals delivered
**Quality Review:** Approved
**Security Review:** Approved

**Issues:**
- [suggestion] [Level 3] implementation-reviewer: Index name deviation idx_parent_type_id vs design spec idx_parent — functionally identical, cosmetic (at: database.py:67)
- [suggestion] [quality] code-quality-reviewer: create-project command name parameter could use slug-derived default (at: commands/create-project.md)
- [suggestion] [quality] code-quality-reviewer: Inline comment for backlog column mapping would aid readability (at: backfill.py:96-97)
- [suggestion] [quality] code-quality-reviewer: double-split optimization in _scan_backlog (at: backfill.py:80-82)
- [suggestion] [security] security-reviewer: Raw exception messages in error strings — acceptable for local tooling (at: server_helpers.py:218)
- [suggestion] [security] security-reviewer: max_depth has no upper bound validation on MCP tool — acceptable for local tooling (at: entity_server.py:181)

**Changes Made:**
- No changes required — final validation round confirmed all prior fixes hold, zero blockers/warnings across all 3 reviewers
