...
- [ ] **T5.2.2: Verify test count and backfill compatibility**
  Verify 18 new test functions added by this feature (15 from design C6 list + `test_update_entity_with_uuid` from T2.5.1 + `test_init_already_migrated_db` from T1.4.4 + `test_process_get_lineage_passes_uuid` from T3.2.2 — the plan's target of 16 omits test_init_already_migrated_db and test_process_get_lineage_passes_uuid; 18 is the correct count). Hard minimum remains 10 per AC-27. Count new tests by matching new function names specifically: `plugins/iflow/.venv/bin/python -m pytest plugins/iflow/hooks/lib/entity_registry/ --collect-only -q | grep -cE 'test_migration_fresh|test_migration_populated|test_migration_populates|test_migration_rollback|test_uuid_immutability|test_self_parent_uuid|test_resolve_identifier|test_register_returns_uuid|test_register_duplicate|test_set_parent_mixed|test_set_parent_updates|test_get_entity_by_uuid|test_get_entity_by_type|test_get_entity_not_found|test_get_lineage_with_uuid|test_export_uses_uuid|test_init_already_migrated|test_update_entity_with_uuid|test_process_get_lineage_passes_uuid'` — expect 18 (tests span test_database.py, test_server_helpers.py, and test_entity_server.py). Verify backfill imports succeed (requires PYTHONPATH):
  ```bash
