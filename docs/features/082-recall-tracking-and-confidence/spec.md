...
- [ ] **AC-20b-1 concurrent-writer (successful wait):** `MemoryDatabase` opens connections with `PRAGMA busy_timeout = 15000` (15s, set at database.py:837). Test uses an override to shorten each connection's busy-timeout for deterministic timing. **Mechanism (authoritative):** construct each `MemoryDatabase` instance first; then execute `db._conn.execute("PRAGMA busy_timeout = 1000")` on each held connection before the test begins. This is a test-only private-attr reach (engineering memory flags `db._conn` as anti-pattern for production code but permits it in test scaffolding with clear intent). Alternative: extend `MemoryDatabase.__init__` with a `busy_timeout_ms: int = 15000` kwarg and call with `busy_timeout_ms=1000`. Implementation chooses one; the spec does not mandate either. Both tests (AC-20b-1 and AC-20b-2) use the same override mechanism. Connection A: `BEGIN IMMEDIATE; INSERT ...; COMMIT` — but insert a `time.sleep(0.1)` BEFORE the COMMIT so A holds the lock ~100ms. Dispatch A via `threading.Thread`. On connection B, invoke `decay_confidence(db_b, config, now=NOW)`. Assert: decay succeeds (no `"error"` key), demotions applied, A's commit visible, no partial UPDATE rolled back. Decay's `BEGIN IMMEDIATE` waits ~100ms for A and then proceeds within the 1s busy-timeout.
