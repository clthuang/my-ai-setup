...
**Inputs:** the description argument; `--prd=<path>` when promoting a brainstorm PRD; `--express` plus an inline mini-spec for the express lane; `list_features` for what is already active.
...
4. `register_entity(entity_type="feature", entity_id="{id}-{slug}", status="planned", ...)` — promotion sources go in `metadata` (brainstorm stem, backlog id, or both) with `parent_uuid` resolved from that source, never into prose. `status="planned"` is what the next step requires.
5. Activate per the workflow-state skill: `activate_feature`, then the feature branch.
6. `--express`: record the mini-spec as a `mini_spec` phase event with metadata `{"text": "<mini-spec>"}`, then hand `/pd:implement` the skipped set `skipped_phases=["brainstorm","specify","design","create-plan"]` for its entry transition. Otherwise continue into `/pd:specify`.
