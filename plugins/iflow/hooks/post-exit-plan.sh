...
# Guard: check config
config_file="${PROJECT_ROOT}/.claude/iflow.local.md"
...
context+="   \`\`\`\n"
context+="   Task tool:\n"
context+="     subagent_type: iflow:task-reviewer\n"
...
if [[ "$has_active" == "yes" ]]; then
    context+="2. Continue with the feature workflow. Run \`/iflow:show-status\` to see the current phase and next steps.\n"
else
    context+="2. Run \`/iflow:wrap-up\` for code review and finishing.\n"
fi
