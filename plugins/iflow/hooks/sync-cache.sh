...
if [[ -f "$INSTALLED_PLUGINS" ]]; then
    # Extract installPath for iflow@my-local-plugins
    CACHE_PLUGIN=$(grep -o '"installPath": *"[^"]*my-local-plugins/iflow/[^"]*"' "$INSTALLED_PLUGINS" 2>/dev/null | head -1 | sed 's/"installPath": *"\([^"]*\)"/\1/' || true)
fi

# Exit gracefully if iflow not found in installed_plugins.json
