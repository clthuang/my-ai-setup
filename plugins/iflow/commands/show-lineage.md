...
| `--feature={id}` (bare 3-digit number) | Look up via `get_entity` MCP tool | If `get_entity(type_id="feature:{id}")` returns not found, glob `{iflow_artifacts_root}/features/{id}-*` to find the full slug, then use `feature:{id}-{slug}` |
...
Hint: Run /iflow:show-status to see known entities, or check if the entity-registry MCP server is running.
```
...
Usage: /iflow:show-lineage --feature=029-entity-lineage-tracking
       /iflow:show-lineage --project=P001
       /iflow:show-lineage --backlog=00019
       /iflow:show-lineage --brainstorm=20260227-054029-entity-lineage-tracking
       /iflow:show-lineage --descendants  (combine with any of the above)
