...
# Iterates all component files under plugins/iflow/ and scores each against
...
# changes, consider using /iflow:promptimize directly.
...
THRESHOLD=80
TIMEOUT_SECS=120
RUBRIC_PATH="plugins/iflow/skills/promptimize/references/scoring-rubric.md"
...
Score all iflow component files against the 10-dimension promptimize rubric.
...
if [[ ! -f "$RUBRIC_PATH" ]]; then
    ...
    echo "This script must be run from the project root (where plugins/iflow/ exists)." >&2
    exit 1
fi
...
while IFS= read -r line; do
    ...
    # Skills: */SKILL.md
    find plugins/iflow/skills -name "SKILL.md" -type f 2>/dev/null | sort
    # Agents: agents/*.md
    find plugins/iflow/agents -maxdepth 1 -name "*.md" -type f 2>/dev/null | sort
    # Commands: commands/*.md
    find plugins/iflow/commands -maxdepth 1 -name "*.md" -type f 2>/dev/null | sort
)

TOTAL=${#FILES[@]}
if [[ "$TOTAL" -eq 0 ]]; then
    echo "Error: No component files found under plugins/iflow/" >&2
    exit 1
fi
