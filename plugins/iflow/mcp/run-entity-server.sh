#!/bin/bash
# Bootstrap and run the MCP entity server.
...
if command -v uv >/dev/null 2>&1; then
    echo "entity-server: bootstrapping venv with uv at $VENV_DIR..." >&2
    uv venv "$VENV_DIR" >&2
...
echo "entity-server: bootstrapping venv with pip at $VENV_DIR..." >&2
