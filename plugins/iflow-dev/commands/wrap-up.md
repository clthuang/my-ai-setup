...
If "Review and complete tasks first": Show "Complete remaining tasks, then run /iflow-dev:wrap-up again." → STOP
...
Task tool call:
  ...
  prompt: |
    ...
    (or vice versa). Update README.md (root). If `plugins/iflow-dev/README.md` exists (dev workspace), update it too.
...
Fix these issues manually, then run /iflow-dev:wrap-up again.
```
