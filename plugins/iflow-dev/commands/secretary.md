...
- Set `workflow_match = "iflow-dev:create-feature"`
...
- Step 7 (DELEGATE) invokes: `Skill({ skill: "iflow-dev:create-feature", args: "{description}" })`
...
9. Set `workflow_match = "iflow-dev:brainstorm"`
...
|---|---|---|
| "debug" / "root cause" / "why is this broken" | iflow-dev:systematic-debugging | 95% |
| "TDD" / "test-driven" / "red-green-refactor" | iflow-dev:implementing-with-tdd | 95% |
| "retrospective" / "retro" / "what went well" | iflow-dev:retrospecting | 95% |
| "update docs" / "sync documentation" | iflow-dev:updating-docs | 95% |
...
3. If NO active feature:
   - If triage set `workflow_match = "iflow-dev:create-feature"` → preserve that route. Explain: "Problem is well-specified. Creating feature directly (skipping brainstorm)."
   - If triage set `workflow_match = "iflow-dev:brainstorm"` → preserve that route. Explain: "No active feature. Starting from brainstorm to ensure proper research and planning."
   - If no triage ran (safety default) → route to `iflow-dev:brainstorm`
