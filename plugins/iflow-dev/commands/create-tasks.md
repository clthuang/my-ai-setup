...
b. **Invoke task-reviewer:** Use the Task tool:
   ...
   Task tool call:
     ...
     subagent_type: iflow-dev:task-reviewer
     prompt: |
...
`/iflow-dev:implement` with `[YOLO_MODE]` in args.
...
AskUserQuestion:
  questions: [{
    ...
    "header": "Next Step",
    "options": [
      {"label": "Continue to /iflow-dev:implement (Recommended)", "description": "Start implementation"},
...
If "Continue to /iflow-dev:implement (Recommended)": Invoke `/iflow-dev:implement`
