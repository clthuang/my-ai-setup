...
Fallback: Read `plugins/iflow-dev/skills/root-cause-analysis/SKILL.md` (dev workspace).
...
Task tool call:
  ...
  subagent_type: iflow-dev:rca-investigator
  prompt: |
...
if [[ -n "$PLUGIN_ROOT" ]] && [[ -x "$PLUGIN_ROOT/.venv/bin/python" ]]; then
  ...
  PYTHONPATH=plugins/iflow-dev/hooks/lib python3 -m semantic_memory.writer \
    --action upsert --global-store ~/.claude/iflow/memory \
    --entry-json '{...}'
fi
```
