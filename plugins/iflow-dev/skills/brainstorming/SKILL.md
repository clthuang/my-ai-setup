...
1. **Research agents (always present):**
   - `{ type: "research", agent: "iflow-dev:internet-researcher", prompt: topic query }`
   - `{ type: "research", agent: "iflow-dev:codebase-explorer", prompt: patterns query }`
   - `{ type: "research", agent: "iflow-dev:skill-searcher", prompt: capabilities query }`
...
- "Route to /root-cause-analysis" → Invoke `Skill({ skill: "iflow-dev:root-cause-analysis" })`
- "Create fix task" → Invoke `Skill({ skill: "iflow-dev:create-feature", args: "--prd={path}" })` with Standard mode
...
| Promote to Feature / Promote Anyway / Promote if crystallised | Ask for mode → Invoke `/iflow-dev:create-feature --prd={current-prd-path}` → STOP |
...
| Route to /root-cause-analysis | Invoke `Skill({ skill: "iflow-dev:root-cause-analysis" })` → STOP |
| Create fix task | Invoke `Skill({ skill: "iflow-dev:create-feature", args: "--prd={current-prd-path}" })` with Standard mode → STOP |
