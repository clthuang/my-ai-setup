...
PLUGIN_COMP_DIR="${PROJECT_ROOT}/plugins/iflow-dev"
...
_count_hardcoded_paths() {
    ...
    while IFS= read -r f; do
        ...
        while IFS= read -r line; do
            ...
            if echo "$line" | grep -q 'plugins/iflow-dev/' 2>/dev/null; then
                if ! _is_fallback_line "$line" "$prev_line"; then
                    ((violations++)) || true
                fi
            fi
            prev_line="$line"
        done < <(grep -B1 'plugins/iflow-dev/' "$f" 2>/dev/null || true)
    done < <(find "$search_dir" -name "$file_pattern" -type f 2>/dev/null)
    echo "$violations"
}

test_no_hardcoded_plugin_paths_in_agents() {
    log_test "No hardcoded plugins/iflow-dev/ in agent .md files (non-fallback)"
...
test_no_hardcoded_plugin_paths_in_skills() {
    log_test "No hardcoded plugins/iflow-dev/ in SKILL.md files (non-fallback)"
...
test_no_hardcoded_plugin_paths_in_commands() {
    log_test "No hardcoded plugins/iflow-dev/ in command .md files (non-fallback, excluding sync-cache)"
...
test_advisors_use_base_directory_derivation() {
    log_test "No raw plugins/iflow-dev/skills/ in advisor files"

    local violations=0
    while IFS= read -r f; do
        [ -z "$f" ] && continue
        local count
        count=$(grep -c 'plugins/iflow-dev/skills/' "$f" 2>/dev/null) || count=0
        violations=$((violations + count))
